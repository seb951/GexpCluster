...
app_server <- function(input, output,session) {
  ...
  observe({
    ...
    output$clustering_plots = renderPlot({
      switch_clustering_plots(multi_k = multi_k, data = data, clustering_metrics = input$clustering_metrics,k = input$k_selected)
    })

    output$hc_plots = renderPlot({
      switch_clustering_plots(multi_k = multi_k_hc, data = data, clustering_metrics = input$hc_metrics,k = input$k_selected_hc)
    })
    ...
    observe({
      ...
      output$mesotable = renderDataTable({clinical2})
